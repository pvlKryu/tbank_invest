...
| **Hand-written models** | `MoneyValue`, `Quotation`, `InvestStreamEvent` | Helpers and streams; REST uses **`V1*`** (see [models-and-dto.md](models-and-dto.md)). |
...
| `models/` | `MoneyValue`, `Quotation`, stream helpers. |
| `services/` | `Invest*Api` one file per gRPC “service” group. |
| `generated/` | `swagger_dart_code_generator` output; **no manual edits**. |

## Not re-exported

Internal implementation details stay private—use generated **`V1*`** or public helpers above.
